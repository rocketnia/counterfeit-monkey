...
Definition: a quip (called the target quip) is peripheral:
	if the target quip is not flagged-ready:
		no;
	if the target quip is unlisted:
		no;
...
Rule for listing matched quips (this is the standard quip disambiguation rule):
	...
	repeat with item running through L:
		...
		change output entry to the item; [* There are easier ways to do this, but I've handcoded it for speed reasons.]
...
Definition: a person (called the chosen person) is ready for transition:
	if the number of entries in the planned conversation of the chosen person is greater than 0:
		let next topic be entry 1 in the planned conversation of the chosen person;
		if the next topic is q-transitional, yes;
	no.
...
Rule for expressing ignorance for the player (this is the no quip worth saying default rule):
	say "You can't think of anything appropriate to say."

Rule for expressing ignorance for someone (called the doofus) (this is the interlocutor ignorance default rule):
	say "'I don't know,' says [the doofus].[run paragraph on]" [This rule is never actually called by default, but it provides a way in should we want to give other characters an I-don't-know response to generic questioning.]

Check asking someone about something (this is the catch fall-throughs of asking rule):
	carry out the expressing ignorance for activity with the player instead;
	say paragraph break.
...
Before conversing when the noun is not the current interlocutor (this is the greet a new interlocutor rule): 
	...
	change last command to the player's command;
	stop the action.
...
last command is indexed text that varies. 
...
To implicitly greet (chosen character - a thing):
	change greeting type to implicit; 
...
To reset the interlocutor:
	...
	change the current quip to generic-quip;
	change the previous quip to generic-quip.
